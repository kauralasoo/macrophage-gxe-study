...
transcript_metadata = rowData(se_ensembl) %>% tbl_df2() %>% 
  dplyr::select(transcript_id, gene_id, ensembl_gene_id, gene_name)

#Find minimal p-values from fastQTL results
naive_fqtl = importFastQTLTable("results/SL1344/salmon_ensembl85/fastqtl_output/naive_100kb_permuted.txt.gz") %>%
  dplyr::rename(transcript_id = gene_id)
ifng_fqtl = importFastQTLTable("results/SL1344/salmon_ensembl85/fastqtl_output/IFNg_100kb_permuted.txt.gz") %>%
  dplyr::rename(transcript_id = gene_id)
sl1344_fqtl = importFastQTLTable("results/SL1344/salmon_ensembl85/fastqtl_output/SL1344_100kb_permuted.txt.gz") %>%
  dplyr::rename(transcript_id = gene_id)
ifng_sl1344_fqtl = importFastQTLTable("results/SL1344/salmon_ensembl85/fastqtl_output/IFNg_SL1344_100kb_permuted.txt.gz") %>%
  dplyr::rename(transcript_id = gene_id)
...
#Identify transcription QTLs
salmon_ensembl_qtl_hits = purrr::map(fastqtl_pvalue_expressed, ~dplyr::group_by(., ensembl_gene_id) %>% 
                               dplyr::mutate(n_transcripts = length(transcript_id)) %>% 
                               dplyr::arrange(ensembl_gene_id, p_beta) %>% dplyr::filter(row_number() == 1) %>% 
                               dplyr::ungroup() %>% dplyr::mutate(p_bonferroni = p_beta * n_transcripts) %>% 
                               dplyr::mutate(p_bonferroni = pmin(p_bonferroni, 1)) %>% 
...
#Call QTLs for the whole transcript quants (no filtering)
#Export complete Ensmebl 85 transcript ratios for FastQTL
se_ensembl = readRDS("results/SL1344/combined_ensembl85_transcript_quants.rds")
transcript_metadata = rowData(se_ensembl) %>% tbl_df2() %>% 
  dplyr::select(transcript_id, gene_id, ensembl_gene_id, gene_name)
...
#Find minimal p-values from fastQTL results
naive_fqtl = importFastQTLTable("results/SL1344/salmon_ensembl85_full/fastqtl_output/naive_100kb_permuted.txt.gz") %>%
  dplyr::rename(transcript_id = gene_id)
ifng_fqtl = importFastQTLTable("results/SL1344/salmon_ensembl85_full/fastqtl_output/IFNg_100kb_permuted.txt.gz") %>%
  dplyr::rename(transcript_id = gene_id)
sl1344_fqtl = importFastQTLTable("results/SL1344/salmon_ensembl85_full/fastqtl_output/SL1344_100kb_permuted.txt.gz") %>%
  dplyr::rename(transcript_id = gene_id)
ifng_sl1344_fqtl = importFastQTLTable("results/SL1344/salmon_ensembl85_full/fastqtl_output/IFNg_SL1344_100kb_permuted.txt.gz") %>%
  dplyr::rename(transcript_id = gene_id)
