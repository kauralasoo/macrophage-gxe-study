...
cut -f1 data/ATAC_Salmonella/genotypes/file_list.txt | python ~/software/utils/submitJobs.py --MEM 1000 --jobname filterVCF --command "python ~/software/utils/vcf/filterVcf.py --sampleList macrophage-chromatin/data/SL1344/ATAC_gt_list.txt --indir ../macrophage-gxe-study/genotypes/GRCh38/imputed_20151005/ --outdir data/ATAC_Salmonella/genotypes/imputed_20151005/ --execute True --vcfSuffix .GRCh38.sorted.vcf.gz"
...
#Merge VCF files
bcftools concat -a chr10.vcf.gz chr11.vcf.gz chr12.vcf.gz chr13.vcf.gz chr14.vcf.gz chr15.vcf.gz chr16.vcf.gz chr17.vcf.gz chr18.vcf.gz chr19.vcf.gz chr1.vcf.gz chr20.vcf.gz chr21.vcf.gz chr22.vcf.gz chr2.vcf.gz chr3.vcf.gz chr4.vcf.gz chr5.vcf.gz chr6.vcf.gz chr7.vcf.gz chr8.vcf.gz chr9.vcf.gz chrX.vcf.gz > imputed.27_samples.vcf

#Sort VCF files
#Sort genotypes (some chr1 snps went to chr9, GATK complains)
bsub -G team170 -n1 -R "span[hosts=1] select[mem>1000] rusage[mem=1000]" -q normal -M 1000 -o sortvcf.%J.jobout "~/software/vcflib/bin/vcfsort imputed.27_samples.vcf > imputed.27_samples.sorted.vcf"

#Run through vcfuniq and get rid of multiallelic SNPs, because liftover might have led to duplicated entries
~/software/vcflib/bin/vcfuniq imputed.27_samples.sorted.vcf | bcftools norm -m+any - | bcftools view -O z -m2 -M2 - > imputed.27_samples.sorted.filtered.vcf.gz
