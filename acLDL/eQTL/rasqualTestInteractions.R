...
#Test interactionss based on QTLTools pvalues
qtltools_min_pvalue = readRDS("results/acLDL/eQTLs/qtltools_min_pvalues.rds")
min_pvalue_hits = lapply(qtltools_min_pvalue, function(x){dplyr::filter(x, p_fdr < 0.1)})
min_pvalues_df = purrr::map_df(min_pvalue_hits, identity, .id = "condition_name") %>%
  dplyr::arrange(phenotype_id, p_nominal)
joint_pairs = dplyr::select(min_pvalues_df, phenotype_id, snp_id) %>% unique() %>%
...
#Test for interactions
interaction_results = testMultipleInteractions(joint_pairs, acldl_list$cqn, acldl_list$sample_metadata, 
                                               filtered_vcf, formula_qtl, formula_interaction, id_field_separator = "-", lme4 = TRUE)
interaction_df = postProcessInteractionPvalues(interaction_results, id_field_separator = "-")
saveRDS(interaction_df, "results/acLDL/eQTLs/lme4_qtltools_interaction_results.rds")
