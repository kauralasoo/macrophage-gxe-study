...
saveFastqtlMatrices(qtltools_list, "results/acLDL/fastqtl_splicing/ensembl_87/", file_suffix = "norm_prop")
...
normalised_fc_list = purrr::map(fc_list, ~replaceNAsWithRowMeans(.) %>% quantileNormaliseRows())
qtltools_list = purrr::map(normalised_fc_list, prepareQTLtoolsMatrix, fastqtl_genepos)
saveFastqtlMatrices(qtltools_list, "results/acLDL/fastqtl_splicing/reviseAnnotations/", file_suffix = "norm_prop")
...
normalised_list = purrr::map(prop_list, ~replaceNAsWithRowMeans(.) %>% quantileNormaliseRows())
fastqtl_norm_prop_list = purrr::map(normalised_list, prepareQTLtoolsMatrix, fastqtl_genepos)
saveFastqtlMatrices(fastqtl_norm_prop_list, "results/acLDL/fastqtl_splicing/leafcutter/", file_suffix = "norm_prop")
...
covariates_list = purrr::map(normalised_list, 
                             ~performPCA(., sample_meta, n_pcs = 6, feature_id = "genotype_id")$pca_matrix %>%
                               dplyr::select(genotype_id, PC1, PC2, PC3, PC4, PC5, PC6) %>%
                               fastqtlMetadataToCovariates())
saveFastqtlMatrices(covariates_list, "results/acLDL/fastqtl_splicing/leafcutter/", file_suffix = "covariates_prop")
...
normalised_fc_list = purrr::map(fc_list, ~replaceNAsWithRowMeans(.) %>% quantileNormaliseRows())
qtltools_list = purrr::map(normalised_fc_list, prepareQTLtoolsMatrix, fastqtl_genepos)
saveFastqtlMatrices(qtltools_list, "results/acLDL/fastqtl_splicing/leafcutter/", file_suffix = "norm_prop")
