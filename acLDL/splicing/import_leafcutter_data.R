...
intron_counts = readr::read_delim("results/acLDL/leafcutter/leafcutter.intron_counts.txt.gz", 
                                  delim = " ", col_names = TRUE)[,sample_ids]
cluster_counts = readr::read_delim("results/acLDL/leafcutter/leafcutter.cluster_counts.txt.gz", 
                                   delim = " ", col_names = TRUE)[,sample_ids]
...
#Filter introns
filtered_introns = intron_matrix[rowSums(intron_matrix >= 1) >= (ncol(intron_counts) - 7),]
...
#Export data to disk
saveRDS(prop_list, "results/acLDL/acLDL_combined_proportions.row_quantile.rds")
