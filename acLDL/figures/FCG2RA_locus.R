...
hits = dplyr::filter(gwas_olaps, gene_name %like% "FCGR2A")

#Extract QTL
leafcutter_qtl = dplyr::filter(trqtl_min_pvalues$leafcutter$Ctrl, group_id == "clu_4324")
leafcutter_tx_names = dplyr::filter(gene_meta_list$leafcutter, gene_id == "clu_4324")$transcript_id
leafcutter_tx = leafcutter_granges[leafcutter_tx_names]

#Extract annotatated transcripts
lipa_tx = exons["ENST00000271450"] %>% removeMetadata()
lipa_cdss = cdss["ENST00000271450"] %>% removeMetadata()

#Make a coverage plot
track_data = wiggleplotrGenotypeColourGroup(str2_df, leafcutter_qtl$snp_id, vcf_file$genotypes, 1)
lipa_coverage = wiggleplotr::plotCoverage(c(lipa_tx, leafcutter_tx), lipa_cdss, track_data = track_data, 
                                          ...
                                          heights = c(0.6,0.4))
ggsave("acLDL_figures/FCGR2A_leafcutter_coverage.unscaled.pdf", lipa_coverage, width = 10, height = 6)

#Make a QTL boxplot
sample_metadata = sample_meta_list$leafcutter %>% 
  dplyr::mutate(condition_name = factor(condition_name, levels = c("Ctrl", "AcLDL")))
data = constructQtlPlotDataFrame(leafcutter_qtl$phenotype_id, leafcutter_qtl$snp_id, 
                                 assays(se_leafcutter)$tpm_ratios, vcf_file$genotypes, 
                                 sample_metadata, 
                                 tbl_df2(rowData(se_leafcutter)) %>% dplyr::mutate(gene_id = transcript_id, gene_name = transcript_id)) %>%
  dplyr::left_join(constructGenotypeText(leafcutter_qtl$snp_id, variant_information), by = "genotype_value")
boxplot = plotQtlRow(data) + 
  ylab("Exon 5 skipping")
ggsave("acLDL_figures/FCGR2A_sQTL_boxplot.pdf", boxplot, width = 4, height = 3)
