...
selected_peaks = dplyr::filter(atac_list$gene_metadata, start > 138680000, end < 138713000, chr == 2)
...
plotting_meta = atac_list$sample_metadata %>%
  ...
  dplyr::mutate(bigWig = file.path("processed/SL1344", sample_id, paste(sample_id, ".bw", sep = ""))) %>%
...
spopl_region = plotCoverage(exons = peak_list, cdss = peak_list, track_data = filtered_tracks, rescale_introns = FALSE, 
                transcript_annotations = peak_annot, fill_palette = c("#a1dab4","#41b6c4","#225ea8"), flanking_length = c(300,300), 
                connect_exons = FALSE, label_type = "peak", plot_fraction = 0.2)
ggsave("results/ATAC/QTLs/figures/SPOPL_region.pdf", spopl_region, width = 10, height = 8)
