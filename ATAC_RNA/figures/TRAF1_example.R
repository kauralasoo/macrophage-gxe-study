...
ggsave("figures/main_figures/TRAF1_boxplot_column.pdf", TRAF1_col_plot, width = 2.5, height = 3.5)
...
tx_plot = plotTranscripts(exons[both_tx$transcript_id[1]], cdss[both_tx$transcript_id[1]], 
...
ggsave("figures/main_figures/TRAF1_manhattan_plot.pdf", joint_plot, width = 5, height = 5)


qtl_summary = importSummariesForPlotting(qtl_df, gwas_stats_labeled, qtl_paths = qtlResults()$rna_rasqual, 
                                         GRCh37_variants = GRCh37_variants, GRCh38_variants = GRCh38_variants, 
                                         cis_dist = 2e5, use_rasqual = TRUE) %>%
  arrange(condition_name, p_nominal) %>% 
  addR2FromLead(vcf_file$genotypes) %>%
  dplyr::filter(condition_name %in% c("RA","naive","IFNg_SL1344")) %>%
  dplyr::rename(track_id = condition_name)
