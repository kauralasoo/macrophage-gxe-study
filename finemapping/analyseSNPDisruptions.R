...
#Import motifs enriched in macrophages
...
motif_disruptions = purrr::map(as.list(cisbp_pwm_enriched), ~quantifyMotifDisruption(., "ATAC_peak_145162", "rs7594476", atac_data$gene_metadata, sequences, snp_info) %>%
                                 dplyr::filter(abs(rel_diff) > 0, max_rel_score > 0.8))
disruption_table = purrr::map_df(motif_disruptions, identity) %>% 
  dplyr::left_join(expressed_motifs, by = "motif_id")

filtered = dplyr::left_join(disruption_table, expressed_motifs) %>% 
