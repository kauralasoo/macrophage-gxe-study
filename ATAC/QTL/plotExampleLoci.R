...
selected_peaks = dplyr::filter(atac_list$gene_metadata, gene_id %in% 
                                 c("ATAC_peak_154955", "ATAC_peak_154957", "ATAC_peak_154958"))
selected_peaks = dplyr::filter(atac_list$gene_metadata, start > region_coords[1], end < region_coords[2], chr == 2)
...
region_pvals = dplyr::filter(peak_pvals, pos > region_coords[1], pos < region_coords[2]) %>%
  dplyr::mutate(track_id = "caQTL")
associated_variants = makeManhattanPlot(region_pvals)
...
ggsave("results/ATAC/QTLs/figures/SPOPL_region_combined.pdf", plot, width = 8, height = 8)
