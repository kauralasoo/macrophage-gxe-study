...
pca_data = dplyr::left_join(pca$pca_matrix, figureNames(), by = "condition_name")
pca_plot = ggplot(pca_data, aes(x = PC1, y = -PC2, color = figure_name, label = sample_id)) + 
  geom_point() + 
  xlab(xlabel) +
  ylab(ylabel) + 
...
ggsave("figures/supplementary//PCA_of_chromatin_accessibility.pdf", pca_plot, width = 4.5, height = 3.5)
